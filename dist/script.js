...
const checkNumInputs = selecor => {
  const numInputs = document.querySelectorAll(selecor); // валидация inputs
...
const forms = state => {
  ...
  (0,_checkNumInputs__WEBPACK_IMPORTED_MODULE_0__["default"])('input[name="user_phone"]'); // сообщения  при отправке
...
const images = () => {
  ...
  workSection.addEventListener('click', e => {
    ...
    if (target && target.matches('div.popup')) {
      imgPopup.style.display = 'none'; //если клик на подложку,скрываем
    }
  });
};
...
const tabs = function (headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', e => {
    const target = e.target; // убираем точку у tabSelector с помощью регулярки и проверяем родителя на клик
...
}); // для сборки проекта gulp build-prod-js
}();
/******/ })()
;
