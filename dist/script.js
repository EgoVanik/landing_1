...
var checkNumInputs = function checkNumInputs(selecor) {
  var numInputs = document.querySelectorAll(selecor); // валидация inputs
...
var forms = function forms(state) {
  var form = document.querySelectorAll('form'),
      inputs = document.querySelectorAll('input'); // валидация inputs

  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_5__["default"])('input[name="user_phone"]'); // сообщения  при отправке
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // создаем div  с сообщением статуса, добавляем класс, встовляем в верстку

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      item.appendChild(statusMessage);
      var formData = new FormData(item); // собираем данные из формы
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  var header = document.querySelector(headerSelector),
      ...
      content = document.querySelectorAll(contentSelector); // скрываем таб и удаляем класс активности
  ...
  hideTabContent();
  showTabContent(); // делегирование событий

  header.addEventListener('click', function (e) {
    var target = e.target; // убираем точку у tabSelector с помощью регулярки и проверяем родителя на клик
...
var timer = function timer(id, deadline) {
  ...
  var getTimeRemaining = function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
        //возвращаем кол-во мс (разницу между датами(от 1.1.1970))
    seconds = Math.floor(t / 1000 % 60),
        //получаем кол-во сек. и округляем
    minutes = Math.floor(t / 1000 / 60 % 60),
        // получаем минуты (деление с остатком)
